    ...
    <div hlmHoverCard>
      ...
      >
        ...
        @if (ocrLoading()) {
          ...
          >
            <div
              class="w-4 h-4 border-2 border-brand/40 border-t-brand rounded-full animate-spin"
            ></div>
            <span class="text-[10px] text-muted-foreground">{{
              'OCR.EXTRACTING' | translate
            }}</span>
          </div>
        }
        @if (editMode()) {
          <div class="flex-1 min-w-0 py-2" (click)="$event.stopPropagation()">
            ...
            <p class="text-[11px] text-muted-foreground mt-1">
              {{ 'CLIPBOARD.EDIT_HINT' | translate }}
            </p>
          </div>
        } @else {
          @if (entry().kind === 'image') {
            <div
              class="shrink-0 w-8 h-8 rounded-md overflow-hidden bg-muted flex items-center justify-center my-2"
            >
              @if (entry().thumbnail) {
                <img
                  [src]="entry().thumbnail!"
            ...
            <div class="flex-1 min-w-0 py-2">
              <p class="text-[13px] font-medium text-foreground leading-snug">
                {{ 'ENTRY.IMAGE' | translate }}
              </p>
